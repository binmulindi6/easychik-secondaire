    ...
    <div class="container flex flex-row justify-between gap-5 mt-16">
        ...
        @if (isset($examens))
            <div class="container p-4">
                ...
                <p class="font-bold text-xl m-4"> Examens
                    {{ $trimestre->nom . ' Annee Scolaire' . $trimestre->annee_scolaire->nom }}</p>
                <table>
                    <thead>
                        ...
                        @foreach ($examens as $item)
                            @if ($item->trimestre_id == $trimestre->id)
                                <tr class="">
                                    ...
                                    <td class="p-1 text-center text-blue-500 unde  rline"><a
                                            href="{{ route('eleves.examens.edit', [$eleve->id, $item->id]) }}">edit</a></td>
                                    <td>
        ...
        @else
            <div class="container p-4">
                ...
                <p class="font-bold text-xl m-4"> Evaluations
                        ...
                        @foreach ($evaluations as $item)
                            @if ($item->periode_id == $periode->id)
                                <tr class="">
                                    ...
                                    <td class="p-1 text-center text-blue-500 unde  rline"><a
                                            href="{{ route('eleves.evaluations.edit', [$eleve->id, $item->id]) }}">edit</a>
                                    </td>
                                    <td>
                                        <form action="{{ route('eleves.destroy', $item->id) }}" method="post">
                                            @csrf
