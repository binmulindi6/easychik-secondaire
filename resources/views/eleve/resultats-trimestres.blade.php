            ...
            <div class="w-full flex flex-row justify-between">
                <div id="bulletin" class="container w-full flex justify-center">
                    @if ($examen != null && $periode1 != null && $periode2 != null)
                        @if (
                            !Auth::user()->isParent() ||
                                (Auth::user()->isParent() && ($resultat1 > 1 || $resultat2 > 1 || $resultatTrim > 1 || $resultatExam > 1)))
                            ...
                            <table id="printable" border-collapse class=" w-full max-w-160 border border-collapse">
                                ...
                                <thead>
                                    ...
                                    @php
                                        ...
                                        <span class="upercase w-full">Classe :
                                            {{ $eleve->classe()->niveau->nom . ' ' . $eleve->classe()->nom }}</span> <br>
                                    </th>
                                </thead>
                                <thead class="">
                                    <th colspan="8"class="uppercase font-bold text-xs py-2">BULLETIN
                                        {{ $trimestre->nom }}, Annee Scolaire {{ $trimestre->annee_scolaire->nom }}</th>
                                </thead>
                                ...
                                <thead>
                                    <th
                                        class="p-1 border font-bold text-left uppercase align-middle bg-transparent border-b shadow-none text-xs  tracking-none whitespace-nowrap text-slate-700 opacity-90">
                                        Cours </th>
                                    <th
                                ...
                                <tbody>
                                    @for ($i = 0; $i < $periode1->count(); $i++)
                                        ...
                                        <tr>
                                            <td
                                                class="p-1 border text-left uppercase align-middle bg-transparent border-b shadow-none text-xs  tracking-none whitespace-nowrap text-slate-700 opacity-90">
                                                {{ $examen[$i]->nom }} </td>
                                            <td
                                                class="p-1 border text-center uppercase align-middle bg-transparent border-b shadow-none text-xs  tracking-none whitespace-nowrap text-slate-700 opacity-90 font-bold">
                                                {{ $periode1[$i]->total }} </td>
                                            <td
                                ...
                                <tfoot>
                                    ...
                                    <tr>
                                        <th
                                            ...
                                            POURCENTAGE </th>
                                        <th
                                            class="p-1 border font-bold text-center uppercase align-middle bg-transparent border-b shadow-none text-xs  tracking-none whitespace-nowrap text-slate-700 opacity-90 bg-slate-400">
                                        </th>
                                        <th
                                            class="p-1 border font-bold text-center uppercase align-middle bg-transparent border-b shadow-none text-xs  tracking-none whitespace-nowrap text-slate-700 opacity-90">
                                            {{ round(($noteP1 * 100) / $maxP1, 1) }} %</th>
                                        <th
                                            class="p-1 border font-bold text-center uppercase align-middle bg-transparent border-b shadow-none text-xs  tracking-none whitespace-nowrap text-slate-700 opacity-90">
                                            {{ round(($noteP2 * 100) / $maxP2, 1) }} %</th>
                                        <th
                                            class="p-1 border font-bold text-center uppercase align-middle bg-transparent border-b shadow-none text-xs  tracking-none whitespace-nowrap text-slate-700 opacity-90 bg-slate-400">
                                        </th>
                                        <th
                                            class="p-1 border font-bold text-center uppercase align-middle bg-transparent border-b shadow-none text-xs  tracking-none whitespace-nowrap text-slate-700 opacity-90">
                                            {{ round(($noteEx * 100) / $maxEx, 1) }} %</th>
                                        <th
                                            class="p-1 border font-bold text-center uppercase align-middle bg-transparent border-b shadow-none text-xs  tracking-none whitespace-nowrap text-slate-700 opacity-90 bg-slate-400">
                                        </th>
                                        <th
                                    ...
                                    <tr>
                                        ...
                                        <th
                                            class="p-1 border font-bold text-center  align-middle bg-transparent border-b shadow-none text-xs  tracking-none whitespace-nowrap text-slate-700 opacity-90">
                                            @if ($conduite1 !== null)
                                                {{ $conduite1->conduite->abbreviation }}
                                            @else
                                                @if (!Auth::user()->isParent())
                                                    <a href="{{ route('conduites.link', [$eleve->id, $p1]) }}"
                                                        class="text-blue-500 underline">conduite indisponible</a>
                                                @else
                                        ...
                                        <th
                                            ...
                                            @else
                                                @if (!Auth::user()->isParent())
                                                    <a href="{{ route('conduites.link', [$eleve->id, $p2]) }}"
                                                        class="text-blue-500 underline">conduite indisponible</a>
                                                @else
                                                    <span class="text-blue-500">conduite indisponible</span>
                                                @endif
                            ...
                            @if (
                                ($resultat1 !== $pourc1 ||
                                    $resultat2 !== $pourc2 ||
                                    $resultatExam !== $pourcExam ||
                                    $resultatTrim !== $pourcTrim) &&
                                    Auth::user()->isEnseignant())
                                <form class="ml-2 self-end"
                                    action="{{ route('resultat.trimestre.store', [$trimestre->id, $eleve->id]) }}"
                                    method="post">
                                    @csrf
                        ...
                        @else
                            <p class="text-bold text-2xl text-red-500 text-center">
                                Buelletin Indisponible 10
                            </p>
                        @endif
                    @else
                        <p class="text-bold text-2xl text-red-500 text-center">
                            Buelletin Indisponible 11
                        </p>
                    @endif
                </div>
            </div>
