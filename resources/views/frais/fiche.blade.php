...
@section('content')
    <div class=" container flex flex-col justify-between gap-5">
        ...
        <div class="frm-identity flex flex-col justify-center items-center  gap-4 shadow-2xl relative bg-white rounded-5 p-5 w-full  z-20">
            ...
            <div class="flex flex-row justify-end w-full">
                <form action="{{route('eleves.paiements.create', $item->id)}}" method="post" >
                    ...
                    <div class="w-60 h-10 flex flex-row justify-end items-center gap-2">
                        <x-select :val="$freq" :collection="$annees" class="block mt-1 w-full" name='frequentation' required></x-select>
                        ...
                        @if (count($paiements) > 0)
                            @foreach ($paiements as $paie)
                                <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700">
                                    ...
                                    <th scope="row" class="px-6 pb-2 font-medium text-gray-900 whitespace-nowrap dark:text-white">
                                        {{$paie->frais->nom}}
                                    </th>
                                    <td class="px-6">
                    ...
                    <table class="">
                        <thead>
                            <tr>
                                ...
                                <th class="text-right border-b">{{$frr['total'] . " " . $frr['frais']->type_frais->devise}}</th>
                            </tr>
                        </thead>
                        <tbody>
                            <tr>
                                <td class="text-left">Montant dû :</td>
                                <td class="text-right">{{$frr['frais']->montant . " " . $frr['frais']->type_frais->devise}}</td>
                            </tr>
                            <tr>
                                <td class="text-left">Solde :</td>
                                <td class="text-right">{{$frr['frais']->montant - $frr['total'] . " " . $frr['frais']->type_frais->devise}}</td>
                            </tr>
                        </tbody>
                    </table>             
                </div>
