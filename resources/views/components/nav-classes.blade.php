<div class="flex flex-col items-center sm:flex-row justify-between gap-4 w-full">
    ...
    <div class="flex flex-col sm:flex-row gap-5">
        @if (
            (str_contains('Classes', $pagename) || $pagename === 'Classes / Edit' || $pagename === 'Classes / Create') &&
    ...
    <div
        class=" classes btn-display items-center flex justify-center gap-2 bg-slate-100 rounded-3 cursor-pointer hover:bg-white px-4 py-2 min-h-10 min-w-30">
...
@endif
@if ($pagename == 'Sections / Create' || $pagename == 'Sections' || $pagename == 'Sections / Edit')
    <div
        class=" btn-create items-center flex justify-center gap-2 bg-slate-100 rounded-3 cursor-pointer hover:bg-white px-4 py-2 min-h-10 min-w-30">
    @else
        <div
            class="btn-create items-center hidden flex justify-center gap-2 bg-slate-100 rounded-3 cursor-pointer hover:bg-white px-4 py-2 min-h-10 min-w-30">
@endif
@if ($pagename != 'Sections / Create' && $pagename != 'Sections')
    <a class="w-full" href="{{ route('sections.create') }}">
        <div>
            <i class="relative top-0 leading-normal text-green-500 fa fa-solid fa-plus text-size-sm"></i>
...
@else
    <div>
        <i class="relative top-0 leading-normal text-green-500 fa fa-solid fa-plus text-size-sm"></i>
        <span class="ml-1 duration-300 opacity-100 pointer-events-none ease">Ajouter une Section</span>
    </div>
@endif
</div>
...
<div
    ...
    @if ($pagename != 'Sections' && $pagename != 'Sections / Create' && $pagename != 'Sections / Edit')
