...
@if (str_contains($pagename, 'Paiements'))
    <div
        class="relative shadow-xl justify-end flex flex-row gap-4 j items-center  max-h-12 transition-all rounded-lg  ease sm-max:text-size-xs sm-max:w-full">
        ...
        @endif
        <form class="w-full pt-2" action="{{ route('paiements.search') }}" method="post" >
            @csrf
            <button type="submit"
            ...
            @if (isset($search))
                <input type="text" name="search" value="{{ $search }}"
                    class=" pt-2 pl-9 text-sm focus:shadow-primary-outline  ease sm-max:text-size-xs w-full leading-5.6 relative -ml-px block min-w-0 flex-auto rounded-lg border border-solid border-gray-300 bg-white bg-clip-padding py-2 pr-3 text-gray-700 transition-all placeholder:text-gray-500 focus:border-blue-500 focus:outline-none focus:transition-shadow"
                    placeholder="Search..." />
            @else
                <input type="text" name="search"
                    class=" pt-2 pl-9 text-sm w-full focus:shadow-primary-outline  ease sm-max:text-size-xs leading-5.6 relative -ml-px block min-w-0 flex-auto rounded-lg border border-solid border-gray-300 bg-white bg-clip-padding py-2 pr-3 text-gray-700 transition-all placeholder:text-gray-500 focus:border-blue-500 focus:outline-none focus:transition-shadow"
                    placeholder="Search..." />
            @endif
        </form>
    </div>
