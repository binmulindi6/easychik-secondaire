...
@section('content')
    <div class="container flex flex-col justify-between gap-5">
        ...
        @if (
            $page_name == 'Entrées / Edit' ||
                $page_name == 'Entrées / Create' ||
                $page_name == 'Entrées / Link' ||
                $errors->any())
        ...
        @if (isset($self))
            ...
            <form method="GET" action="{{ route('entrees.show', $self->id) }}">
                @csrf
                {{ method_field('PUT') }}
                <!-- Email Address -->
        ...
        @else
            <span class="font-bold text-base"> Enregistrer une Entrée en Stock </span>
            <x-errors class="mb-4" :errors="$errors" />
            <form method="POST" action="{{ route('entrees.store') }}">
                ...
                <div class="flex justify-between items-center md:gap-4 md-max:flex-col ">
                    <div class="mt-4 w-full">
                        <x-label for="quantity" :value="__('Quantité')" />

                        <x-input id="quantity" class="block mt-1 w-full" type="number" name="quantite" :value="old('quantite')"
                            value='0' placeholder="ex: 10 ou 0" required autofocus />
                    </div>
                    <div class="mt-4 w-full">
                            <x-label for="quantity" :value="__('Prix Achat Unitaire')" />
                            <div class="flex w-full items-center gap-2">
                                <x-input id="quantity" class="block mt-1 w-8/12" type="number" name="prix" min='1'
                                    :value="old('prix')" value='1' placeholder="ex: 10 ou 20" required autofocus />
                                <select name="devise" id="nom"
                ...
                <div class="flex justify-between gap-4 smsm-max:flex-col">
                    ...
                    <div class="mt-4 w-full">
                        ...
                        <x-input id="quantity" class="block mt-1 w-full" type="date" name="date" max="{{$today}}"
                            :value="old('date')" placeholder="Achat materiel de bureau" required autofocus />
                    </div>
                </div>
                <div class="mt-4">
                    <x-button>Ajouter au Stock</x-button>
                </div>
    ...
    @if (isset($items) && $items->count() > 0)
        <div class="p-4 pb-0 mb-0 bg-white rounded-t-2xl">
            <p class="font-bold text-xl">Les Entrées des Materiels</p>
        </div>
                            ...
                            <tr class="">
                                ...
                                <td
                                    ...
                                    <div class="flex justify-center gap-4 align-middle">
                                        <a title="edit" href="{{ route('entrees.edit', $item->id) }}"><i
                                                class="fa fa-solid fa-pen"></i></a>
                                        <form action="{{ route('entrees.destroy', $item->id) }}" method="post">
                                            @csrf
                                            @method('DELETE')
