...
@section('content')
    <div class="container flex flex-col justify-between gap-5">
        @if (isset($search))
            @if (isset($classe))
                <x-nav-cotation-travails :search="$search" :pagename="$page_name" :eleves="true"
                    :classe="$classe" :evaluation="$evaluation"></x-nav-cotation-travails>
            @else
                <x-nav-cotation-travails :search="$search" :pagename="$page_name" :eleves="true"
                    :evaluation="$evaluation"></x-nav-cotation-travails>
            @endif
        @else
            @if (isset($classe))
                <x-nav-cotation-travails :classe="$classe" :pagename="$page_name" :eleves="true" :evaluation="$evaluation"></x-nav-cotation-travails>
            @else
                <x-nav-cotation-travails :pagename="$page_name" :eleves="true" :evaluation="$evaluation"></x-nav-cotation-travails>
            @endif
        @endif
        ...
        @if (isset($items))
            ...
            <div class=" flex justify-center p-4 pb-0 mb-0 bg-white rounded-t-2xl">
                <h6 class="text-center uppercase">
                    {{ $evaluation->type_evaluation->nom . ' ' . $evaluation->cours->nom . ' ' . $evaluation->periode->nom . ' ' . $evaluation->periode->trimestre->nom . ' ' . $evaluation->periode->trimestre->annee_scolaire->nom }}
                </h6>
            </div>
            @if (count($items) <= 0)
                                    ...
                                    @if (Auth::user()->isEnseignant())
                                        ...
                                        <tr id="tr{{ $item->id }}" class="rounded-2xl hover:bg-slate-100">
                                            <td
                                                ...
                                                <a href="{{ route('eleves.show', $item->eleve_id) }}"
                                            ...
                                            <td
                                                class="p-1 text-size-sm text-center align-middle bg-transparent border-b  shadow-transparent ">
                                                <form id="frm{{ $item->id }}"
                                                    action="{{ route('eleves.evaluations.update.api', $item->id) }}"
                                                    method="POST" class="flex justify-center items-center gap-2">
                                                    @method('PUT')
                                                    @csrf
                                                    <x-input id="ev{{ $item->id }}"
                                                        class="px-2 py-2 rounded w-20 text-center h-8" type="number"
                                                        max='{{ $evaluation->note_max }}' name="note_obtenu"
