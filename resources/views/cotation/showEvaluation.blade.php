...
@section('content')
    <div class="container flex flex-col justify-between gap-5">
        @if (isset($search))
            <x-nav-cotation-travails :search="$search" :pagename="$page_name" :eleves="true" :evaluation="$evaluation"></x-nav-cotation-travails>
        @else
    ...
    @if (isset($items))
        ...
        <div class=" flex justify-center p-4 pb-0 mb-0 bg-white rounded-t-2xl">
            <h6 class="text-center uppercase">{{$evaluation->type_evaluation->nom . " " . $evaluation->cours->nom . " " . $evaluation->periode->nom . " " . $evaluation->periode->trimestre->nom . " " . $evaluation->periode->trimestre->annee_scolaire->nom }}</h6>
        </div>
        @if (count($items) <= 0)
